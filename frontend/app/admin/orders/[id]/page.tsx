      ...
      <RouteGuard requireAdmin>
        <div className="p-12 text-center text-muted-foreground">
          ...
          <p className="mt-2 text-sm">
            {error || "The order you are looking for doesn't exist."}
          </p>
          <Link href="/admin/orders">
            <Button
              variant="link"
              className="mt-4 rounded-none text-[10px] uppercase tracking-widest font-bold"
            >
              Back to Orders
            </Button>
          </Link>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold tracking-tight flex items-center gap-3 font-serif italic">
                ...
                <Badge
                  variant="outline"
                  className={`border text-[10px] uppercase tracking-widest font-bold rounded-none ${getStatusColor(order.status)}`}
                >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <Card className="rounded-none border-border/50">
              ...
              <CardContent className="space-y-6">
                {order.items?.map((item: any, idx: number) => (
                  <div
                    key={idx}
                    className="flex gap-4 items-start pb-6 border-b border-border/50 last:border-0 last:pb-0"
                  >
                    ...
                    <div className="flex justify-between text-green-600">
                      <div className="flex items-center gap-2">
                        ...
                        {order.coupon_code && (
                          <Badge
                            variant="secondary"
                            className="text-[10px] uppercase font-bold tracking-widest h-5 px-1.5 bg-green-500/10 text-green-600 border-green-200 rounded-none"
                          >
                            {order.coupon_code}
                          </Badge>
                        )}
                ...
                {latestTransaction && (
                  <div className="flex justify-between text-sm items-center py-2 border-b border-border/50">
                    ...
                    <span className="flex items-center gap-2 font-medium font-serif italic">
                      {latestTransaction.payment_method === "MPESA" && (
          ...
          <div className="space-y-8">
            ...
            {latestTransaction && latestTransaction.response_data && (
              <Card className="rounded-none border-border/50">
                <CardHeader>
                  <CardTitle className="font-serif italic text-base">
                    Payment Logs
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="space-y-2 text-xs font-mono bg-black/5 dark:bg-black p-4 border border-border/50 rounded-none w-full break-all overflow-hidden text-muted-foreground">
                    {typeof latestTransaction.response_data === "object" ? (
                      Object.entries(latestTransaction.response_data).map(
                        ([key, value]) => (
                          <div
                            key={key}
                            className="flex flex-col sm:flex-row justify-between gap-1 sm:gap-4 overflow-hidden border-b border-border/30 last:border-0 pb-1 last:pb-0"
                          >
