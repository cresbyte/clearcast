...
export default function OrderDetails({ params }: OrderDetailsProps) {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case "P":
        return "bg-yellow-500/10 text-yellow-600 hover:bg-yellow-500/20 border-yellow-200";
      case "A":
        return "bg-blue-500/10 text-blue-500 hover:bg-blue-500/20 border-blue-200";
      case "S":
        return "bg-orange-500/10 text-orange-500 hover:bg-orange-500/20 border-orange-200";
      case "D":
        return "bg-green-500/10 text-green-500 hover:bg-green-500/20 border-green-200";
      case "C":
        return "bg-destructive/10 text-destructive hover:bg-destructive/20 border-destructive-200";
      default:
  ...
  if (error || !order) {
    return (
      <div className="p-12 text-center text-muted-foreground">
        ...
        <p className="mt-2">
          The order you are looking for doesn't exist or you don't have access
          to it.
        </p>
        <Link
          href="/profile/orders"
    ...
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12 max-w-4xl">
      <div className="mb-7">
        <Link href="/profile/orders">
          <Button
            variant="ghost"
            className="pl-1 hover:pl-2 transition-all rounded-none"
          >
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          <h1 className="text-3xl font-serif font-bold text-foreground flex items-center gap-3">
            Order #{order.id}
            <Badge
              className={`text-base px-3 py-1 rounded-none ${getStatusColor(order.status)}`}
            >
              {getStatusDisplay(order.status)}
            </Badge>
          </h1>
          <p className="text-muted-foreground mt-2 flex items-center gap-2">
            <Clock className="w-4 h-4" /> Placed on{" "}
              ...
              <div className="relative border-l-2 border-muted ml-2 space-y-8 pb-2">
                {timeline.map((event, idx) => (
                  <div key={idx} className="ml-8 relative">
                    <div
                      className={`absolute -left-[41px] top-1 h-4 w-4 rounded-full border-2 border-background ${event.completed ? "bg-primary" : "bg-muted"}`}
                    />
                ...
                <div key={idx} className="flex gap-4 items-start">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start">
                      <div>
                        <p className="font-semibold text-base truncate pr-4 text-serif">
                          {item.product_name}
                        </p>
                        {item.variant_name && (
                  ...
                  <div className="flex justify-between text-green-600">
                    <div className="flex items-center gap-2">
                      ...
                      {order.coupon_code && (
                        <Badge
                          variant="secondary"
                          className="text-xs h-5 px-1.5 bg-green-100 text-green-700 hover:bg-green-100 border-green-200 rounded-none"
                        >
                          {order.coupon_code}
                        </Badge>
                      )}
        ...
        <div className="space-y-6">
          <Card className="rounded-none">
            ...
            <CardContent className="space-y-6">
              ...
              <div>
                <h3 className="font-semibold mb-2 flex items-center gap-2 text-[10px] text-muted-foreground uppercase tracking-wider">
                  {getPaymentIcon(latestTransaction?.payment_method)} Payment
                  Method
                </h3>
